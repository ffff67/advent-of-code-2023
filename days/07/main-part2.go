...
func (p Player) handType() int {
	...
	highestCountOfAny := getHighestCountInMap(cardMap)
	wildCardCombo := highestCountOfAny + jokerCount

	if wildCardCombo == 5 {
		return FiveOfAKind
	} else if wildCardCombo == 4 {
		return FourOfAKind
	} else if wildCardCombo == 3 {
		if (jokerCount == 3 && anyInMap(cardMap, 2)) ||
			(jokerCount == 2 && anyInMapExcludingJoker(cardMap, 2)) ||
			(jokerCount == 1 && !anyInMapExcludingJoker(cardMap, 1)) ||
			(jokerCount == 0 && anyInMap(cardMap, 2)) {
			return FullHouse
		} else {
			return ThreeOfAKind
		}
	} else if wildCardCombo == 2 {
...
func anyInMapExcludingJoker(m map[rune]int, val int) bool {
	for k, v := range m {
		if k != 'J' && v == val {
			return true
		}
...
func getHighestCountInMap(m map[rune]int) int {
	var max int
	for k, v := range m {
		if k != 'J' && v > max {
			max = v
